...
function ENT:OnRemove()
	...
	local object = self:GetObject()
	if object:IsValid() then
		if object.PREV_RENDER_MODE then
			object:SetRenderMode(object.PREV_RENDER_MODE)
			object.PREV_RENDER_MODE = nil
		end
		local col = object:GetColor()
		col.a = 255
		object:SetColor(col)
...
local ShadowParams = {secondstoarrive = 0.01, maxangular = 500, maxangulardamp = 10000, maxspeed = 500, maxspeeddamp = 1000, dampfactor = 0.65, teleportdistance = 0}
...
function ENT:Think()
	...
	if owner:KeyPressed(IN_ATTACK) then
		...
		self:Remove()
		return
	elseif owner:KeyPressed(IN_USE) then
		object:SetPhysicsAttacker(owner)
	...
	elseif owner:KeyDown(IN_ATTACK2) and not owner:GetActiveWeapon().NoPropThrowing then
		...
		if owner:KeyDown(IN_WALK) then
			local id = id or GAMEMODE:GetHintByType("proprotate")
			owner:DispatchHint(id)

			if owner:KeyDown(IN_SPEED) then
				--first lets normalize our angle
				--we only need to worry about pitch and yaw, roll would just be annoying, but we'll just zero it anyway
				--i doubt people will use weird object orientations if they want to align it to world anyway
				local iInterval = 45
				local ang = self.ObjectAngles
				--ang.r = 0
				ang:SnapTo("r", iInterval)
				ang:SnapTo("p", iInterval)
				ang:SnapTo("y", iInterval)

				--now we check our rotational "buildups" to see if we should progress the rotation at all
				owner.MouseXBuildup = (owner.MouseXBuildup or 0) + ((owner.InputMouseX or 0) / 2)
				owner.MouseYBuildup = (owner.MouseYBuildup or 0) + ((owner.InputMouseY or 0) / 2)

				local iSnapX = math.Round(owner.MouseXBuildup / iInterval) * iInterval
				local iSnapY = math.Round(owner.MouseYBuildup / iInterval) * iInterval
				--ang.y = ang.y + iSnapX
				--ang.p = ang.p + iSnapY
				ang:RotateAroundAxis(owner:EyeAngles():Up(), iSnapX)
				ang:RotateAroundAxis(owner:EyeAngles():Right(), iSnapY)
				--clean up the buildups if there were any changes
				if iSnapX ~= 0 then
					owner.MouseXBuildup = 0
				end

				if iSnapY ~= 0 then
					owner.MouseYBuildup = 0
				end
			else
		...
		local correctedObj = correctedObj or false
		if object:IsValid() then
			local col = object:GetColor()
			col.a = 178
			if not object.PREV_RENDER_MODE then object.PREV_RENDER_MODE = object:GetRenderMode() end
