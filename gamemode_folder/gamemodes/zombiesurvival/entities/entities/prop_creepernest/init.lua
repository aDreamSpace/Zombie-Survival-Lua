...
function ENT:BuildUp()
	...
	if self.BuildsThisTick < 3 then
		...
		self:SetNestHealth(math.min(self:GetNestHealth() + FrameTime() * self:GetNestMaxHealth() * 0.125, self:GetNestMaxHealth()))
	end
end
...
function ENT:OnTakeDamage(dmginfo)
	...
	self:SetNestHealth(self:GetNestHealth() - dmginfo:GetDamage() * 2)
