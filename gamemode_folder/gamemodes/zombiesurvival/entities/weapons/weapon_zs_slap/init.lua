...
function SWEP:RemoveGhost()
	local owner = self.Owner
	if owner and owner:IsValid() then
		owner:RemoveStatus("ghost_slap", false, true)
	end
end
...
function SWEP:PrimaryAttack()
	...
	local status = owner.status_ghost_slap
	...
	local ent = ents.Create("prop_slap")
	if ent:IsValid() then
		ent:SetPos(pos)
		ent:SetAngles(ang)
		ent:Spawn()
		...
		if not owner:HasWeapon("weapon_zs_slapremote") then
			owner:Give("weapon_zs_slapremote")
		end
		owner:SelectWeapon("weapon_zs_slapremote")
	end
end
