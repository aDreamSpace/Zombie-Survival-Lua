...
function SWEP:SpawnGhost()
	...
	if owner and owner:IsValid() then
		owner:GiveStatus("ghost_miniturret")
	end
end
...
function SWEP:RemoveGhost()
	local owner = self.Owner
	if owner and owner:IsValid() then
		owner:RemoveStatus("ghost_miniturret", false, true)
	end
end
...
function SWEP:PrimaryAttack()
	...
	local status = owner.status_ghost_miniturret
	...
	local ent = ents.Create("prop_miniturret")
	if ent:IsValid() then
		ent:SetPos(pos)
		ent:SetAngles(ang)
		ent:Spawn()
