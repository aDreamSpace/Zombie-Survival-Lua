...
function SWEP:Think()
	...
	if SERVER then
		if self.ToSpit > 0 and self.SpitDelay < curtime then
			self.ToSpit = self.ToSpit - 1
			self.SpitDelay = curtime + 0.3
	...
	if self:IsGoingToSpit() and self:GetNextSpit() <= curtime then
		...
		if SERVER then
			...
			--if we have the 3 spit trait then we need to do this twice more
			if owner:GetZTraitCurrent("3spit") then
				self.ToSpit = 2
				self.SpitDelay = curtime + 0.3
			end
		end
...
function SWEP:PrimaryAttack()
	...
	local LeapDelay = 1.25
	if owner:GetZTraitCurrent("qleap") then
		LeapDelay = LeapDelay * 0.7
	end

	self:SetNextLeap(CurTime() + LeapDelay)
