...
function SWEP:SpawnGhost()
	...
	if owner and owner:IsValid() then
		owner:GiveStatus("ghost_medstation")
	end
end
...
function SWEP:RemoveGhost()
	local owner = self.Owner
	if owner and owner:IsValid() then
		owner:RemoveStatus("ghost_medstation", false, true)
	end
end
...
function SWEP:PrimaryAttack()
	...
	local status = owner.status_ghost_medstation
	...
	local ent = ents.Create("prop_medstation")
	if ent:IsValid() then
		ent:SetPos(pos)
		ent:SetAngles(ang)
		ent:Spawn()
