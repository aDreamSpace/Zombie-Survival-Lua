...
SWEP.MeleeReach = 50
SWEP.MeleeDamage = 7
SWEP.MeleeForceScale = 10
SWEP.MeleeSize = 1.5
SWEP.MeleeDamageType = DMG_SLASH
SWEP.Primary.Delay = 0.2

SWEP.PounceDamage = 15 --SWEP.PounceDamage = 10
...
function SWEP:Think()
	...
	--getting the ztraits is rather expensive since it has to lookup the class table
	--so instead we'll just cache it on swep in the think function
	if self.NextTraitCheck < curtime then
		self.NextTraitCheck = curtime + 1

		self.FastClimb = owner:GetZTraitCurrent("fclimb")
		self.PlusForce = owner:GetZTraitCurrent("flunge")
	end
		...
		if self:GetClimbSurface() and owner:KeyDown(IN_ATTACK2) then
			if SERVER and curtime >= self.NextClimbSound then
				...
				if speed >= 50 then
					...
					owner:EmitSound("player/footsteps/metalgrate"..math.random(4)..".wav", 80, 60)
				end
			end
		else
			self:StopClimbing()
			...
			for _, trace in ipairs(traces) do
				...
				if trace.HitWorld then
					...
					if ent and ent:IsValid() then
						hit = true
						local hforce = 1
						if self.PlusForce then
							hforce = 10
						end
						self:MeleeHit(ent, trace, damage, ent:IsPlayer() and hforce or 10)
					end
				end
			end

			if SERVER and hit then
				owner:EmitSound("physics/flesh/flesh_strider_impact_bullet1.wav", 80, 60)
				owner:EmitSound("npc/fast_zombie/wake1.wav", 80, 60)
			end
...
function SWEP:SecondaryAttack()
	...
	if self.Owner:IsOnGround() then
		...
		if SERVER then
			self.Owner:EmitSound("npc/fast_zombie/leap1.wav", 80, 60)
		end
...
function SWEP:StartPounce()
	...
	if owner:IsOnGround() then
		...
		if SERVER then
			owner:EmitSound("NPC_FastZombie.Scream", 80, 60)
		end
...
function SWEP:PlayHitSound()
	self.Owner:EmitSound("NPC_FastZombie.AttackHit", 80, 60)
end

function SWEP:PlayMissSound()
	self.Owner:EmitSound("NPC_FastZombie.AttackMiss", 80, 60)
end
...
function SWEP:PlayIdleSound()
	self.Owner:EmitSound("NPC_FastZombie.AlertFar", 80, 60)
	self:SetRoarEndTime(CurTime() + self.RoarTime)
end

function SWEP:PlayAlertSound()
	self.Owner:EmitSound("NPC_FastZombie.Frenzy", 80, 60)
	self:SetRoarEndTime(CurTime() + self.RoarTime)
end

function SWEP:StartSwingingSound()
	self.Owner:EmitSound("NPC_FastZombie.Gurgle", 80, 60)
end

function SWEP:StopSwingingSound()
	self.Owner:StopSound("NPC_FastZombie.Gurgle", 80, 60)
end
...
function SWEP:Move(mv)
	...
	elseif self:GetClimbing() then
		...
		if self.FastClimb then
			velAmt = velAmt * 1.25
		end

		if owner:KeyDown(IN_FORWARD) then
			vel = vel + dir * velAmt --160
		end
		if owner:KeyDown(IN_BACK) then
			vel = vel + dir * (velAmt * -1) ---160
		end
