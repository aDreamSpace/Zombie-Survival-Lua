...
local colBeam= Color(100, 255, 100)
...
function ENT:DrawTranslucent()
	...
	local tr = util.TraceLine({start = epos, endpos = epos + LightNrm * (1024), mask = MASK_SHOT})
	render.SetMaterial(matBeam)
	render.DrawBeam(epos, tr.HitPos, 1, 0, 1, COLOR_WHITE)
	render.DrawBeam(epos, tr.HitPos, 4, 0, 1, colBeam)
	render.SetMaterial(matGlow)
	render.DrawSprite(epos, 4, 4, COLOR_WHITE)
	render.DrawSprite(epos, 16, 16, colBeam)
...
function ENT:CalcView(pl, origin, angles, fov, znear, zfar)
	...
	return {origin = tr.HitPos + angles:Right() * 20 + tr.HitNormal * 3}
end
