...
ENT.ControllerClassName = "weapon_zs_gunturretcontrol"
ENT.SWEPClassName = "weapon_zs_gunturret"
ENT.SWEPAmmoName = "thumper"

ENT.SoundPerShot = false
...
function ENT:IsValidTarget(target)
	return (target:IsPlayer() and target:Team() == TEAM_UNDEAD and target:Alive() or target:GetClass():StartWith("nz_")) and self:GetForward():Dot(self:GetAnglesToTarget(target):Forward()) >= self.MinimumAimDot and TrueVisible(self:ShootPos(), self:GetTargetPos(target), self)
end
...
function ENT:GetManualControl()
	...
	if owner:IsValid() and owner:Alive() and owner:Team() == TEAM_HUMAN then
		...
		if wep:IsValid() and wep:GetClass() == self.ControllerClassName and wep.GetTurret and wep:GetTurret() == self and not wep:GetDTBool(0) then
			return true
		end
	end
