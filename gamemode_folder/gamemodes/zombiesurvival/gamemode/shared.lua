...
--these extensions MUST be loaded before everything else
...
GM.SoundDuration = {
	...
	["gentclub/lasthumanv2.ogg"] = 242.514,
...
function GM:SortZombieSpawnDistances(allplayers)
	...
	for _, pl in pairs(allplayers) do
		...
		elseif CLIENT or pl.LastNotAFK and CurTime() <= pl.LastNotAFK + (GAMEMODE.WaveZeroLength - 20) then
...
function GM:GetDesiredStartingZombies()
	...
	if GAMEMODE.UseMinPlayersForPVP and numplayers < GAMEMODE.MinPlayersForPVP then
		if navmesh.IsLoaded() then --make sure there is atleast something to fight first...
			return 0
		end
	end
...
--cache some meta functions since metatable lookup is SO SLOW -MagicSwap
local r = debug.getregistry()
local CShouldNotCollide = r.Player.ShouldNotCollide
	...
	if math.floor(damage) > 0 then
		...
		if SERVER then
			if damage >= 30 and damage < pl:Health() then
				pl:KnockDown(math.Clamp((damage * 0.10), 0, 90))
			end
...
--experimental optimizations, test later -MagicSwap
function GM:PlayerCanHearPlayersVoice(listener, talker)
	--if cvars.Number("sv_alltalk", 0) > 0 then return true end
	if GAMEMODE.m_TempAllTalk then return true end

	return CIsValid(listener) and CIsValid(talker) and CTeam(listener) == CTeam(talker) or CTeam(listener) == TEAM_SPECTATOR
...
function GM:ScalePlayerDamage(pl, hitgroup, dmginfo)
	...
	if not pl:CallZombieFunction("ScalePlayerDamage", hitgroup, dmginfo) and pl:Team() == TEAM_UNDEAD then

		--dmginfo:SetDamage(dmginfo:GetDamage() * (HitGroupMult[hitgroup] or 1))
	...
	--there's ALOT of networking that is prevented with this janky fix, its shared so there should be little desync
	if not pl.SpawnProtection then
		if (hitgroup == HITGROUP_LEFTLEG or hitgroup == HITGROUP_RIGHTLEG) and self:PlayerShouldTakeDamage(pl, dmginfo:GetAttacker()) then
			pl.LegDamage = CurTime() + math.min(GAMEMODE.MaxLegDamage, (math.max(0, (pl.LegDamage or 0) - CurTime()) + dmginfo:GetDamage()) * 0.0625) --0.125)
		end
	end
end
...
--]]
--workaround for nextbots
	local entCache = entCache or {}

	if table.Count(entCache) > 0 and stale > CurTime() then
		for index, team in pairs(entCache) do
			local ent = ents.GetByIndex(index)
			if ent:IsValid() and team == teamid then
				local dist = ent:NearestPoint(pos):Distance(pos)
				if dist <= FEAR_RANGE then
					power = power + ((FEAR_RANGE - dist) / FEAR_RANGE) * (ent.FearPerInstance or ent:GetZombieClassTable().FearPerInstance or FEAR_PERINSTANCE)
				end
			end
		end
	end
	...
	if stale <= CurTime() then
		entCache = {}
		for _, ent in pairs(player.GetAll()) do
			if ent ~= ignore and ent:Team() == teamid and not ent:CallZombieFunction("DoesntGiveFear") and ent:Alive() then
				local dist = ent:NearestPoint(pos):Distance(pos)
				if dist <= FEAR_RANGE then
					power = power + ((FEAR_RANGE - dist) / FEAR_RANGE) * (ent:GetZombieClassTable().FearPerInstance or FEAR_PERINSTANCE)
					entCache[ent:EntIndex()] = ent:Team()
				end
			end
		end
		stale = CurTime() + 10
...
function GM:IsSpecialPerson(pl, image)
	...
	if pl:SteamID() == "STEAM_0:1:24450145" then
		img = "icon16/wand.png"
		tooltip = "Server Developer"
	elseif pl:SteamID() == "STEAM_0:1:50105771" then
		img = "icon16/ruby.png"
		tooltip = "Value Changer"
    elseif pl:IsUserGroup( "donator" ) then
		img = "icon16/heart.png"
		tooltip = "Donator"
	elseif pl:IsUserGroup( "contentcreator" ) then
		img = "icon16/user.png"
		tooltip = "Content Creator"
	elseif pl:CheckGroup("moderator") then
		img = "icon16/star.png"
		tooltip = "Staff"
	end
