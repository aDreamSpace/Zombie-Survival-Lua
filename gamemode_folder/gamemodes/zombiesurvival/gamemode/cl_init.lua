...
include("vgui/dhint.lua")
include("vgui/dwebimage.lua")
		...
		if ent:IsValid() and not ent:IsPlayer() and (ent:GetMoveType() == MOVETYPE_NONE or ent:GetMoveType() == MOVETYPE_VPHYSICS) and ent:GetSolid() == SOLID_VPHYSICS then
			if mc == MOUSE_LEFT then
				if ent == self.CraftingEntity then
					...
					local cent2 = MySelf:GetActiveWeapon()
					RunConsoleCommand("_zs_craftcombine", self.CraftingEntity:EntIndex(), cent2:EntIndex())
				end
...
local bHalloweenFog = false
function GM:SetupWorldFog()
	if bHalloweenFog then
		render.FogMode(1)

		render.FogStart(0)
		render.FogEnd(512)
		render.FogColor(40, 40, 40)
...
function GM.PostPlayerDrawMedical(pl)
	if not skip and pl:Team() == TEAM_HUMAN and pl ~= MySelf then
		local eyepos = EyePos()
...
local colSpr = Color(0, 255, 0)
CreateClientConVar("gc_drawfearmeter", "1", true, false, "Sets whether to show the Fear-O-Meter.\n0 = Don't show Fear-O-Meter.\n1 = Show Fear-O-Meter.")
hook.Add("AddExtraOptions", "FearMeterOption", function(list, Window)
	local check = vgui.Create("DCheckBoxLabel", Window)
	check:SetText("Display Fear-O-Meter")
	check:SetConVar("gc_drawfearmeter")
...
local pulse
local pulsesize
local strResist = "resist_x"
local strCvar = "gc_drawfearmeter"
local obs
local rot
local sy
local size
...
function GM:DrawFearMeter(power, screenscale)
	...
	if currentpower >= 0.75 then
		...
		if pulse > 0 then
			pulse = pulse ^ 2
			pulsesize = pulse * screenscale * 28
	...
	surface_SetDrawColor(160, 160, 160, 200)
	rot = math.Clamp((0.5 - currentpower) + math.sin(RealTime() * 10) * 0.01, -0.5, 0.5) * 300
	surface_DrawTexturedRectRotated(w * 0.5 - math.max(0, rot * wid * -0.0001), h - hei * 0.5 - math.abs(rot) * hei * 0.00015, wid, hei, rot)

	surface.SetMaterial(matEyeGlow)
	colSpr.r = 0 + 255 * currentpower
	colSpr.g = 255 - 255 * currentpower
	...
	if MySelf:Team() == TEAM_UNDEAD then
		...
		if currentpower > 0 and not self.ZombieEscape then
			draw_SimpleTextBlurry(translate.Format(strResist, math.ceil(self:GetDamageResistance(currentpower) * 100)), "ZSDamageResistance", w * 0.5, my + hei * 0.75, Color(currentpower * 200, 200 - currentpower * 200, 0, 255), TEXT_ALIGN_CENTER)
		end
	end
end

local strDynamicSpawnDisabled = "DynamicSpawningDisabled"
function GM:GetDynamicSpawning()
	return not GetGlobalBool(strDynamicSpawnDisabled, false)
end
...
GM.TIMEUNTILNEXTPLAYERCACHE = 0
local tPlys
function GM:CGetAllPlayers()
	if self.TIMEUNTILNEXTPLAYERCACHE < CurTime() then
		self.TIMEUNTILNEXTPLAYERCACHE = CurTime() + 0.5
		tPlys = player.GetAll()
	end
	return tPlys
end
...
local endtime
local timleft
local strSound = "buttons/lightswitch2.wav"
local myteam
local tab
local wep
function GM:_Think()
	--unused, disabled for now
	--[[ 
	...
	endtime = self:GetWaveActive() and self:GetWaveEnd() or self:GetWaveStart()
	if endtime ~= -1 then
		timleft = math.max(0, endtime - CurTime())
		if timleft <= 10 and lastwarntim ~= math.ceil(timleft) then
			lastwarntim = math.ceil(timleft)
			if 0 < lastwarntim then
				surface_PlaySound(strSound)
			end
		end
	end
	...
	for _, pl in ipairs(self:CGetAllPlayers()) do
		if pl:IsValid() and pl:Team() == TEAM_UNDEAD then
...
local strBeatDisabled = "beatsdisabled"
function GM:ShouldPlayBeats(teamid, fear)
	return not self.RoundEnded and not self.ZombieEscape and not GetGlobalBool(strBeatDisabled, false)
end

local cv_ShouldPlayMusic = CreateClientConVar("zs_playmusic", 1, true, false)
local NextBeat = 0
local LastBeatLevel = 0
local strShouldPlayBeats
function GM:PlayBeats(teamid, fear)
	if RealTime() <= NextBeat or not gamemode.Call(strShouldPlayBeats, teamid, fear) then return end
...
local w, h
local packup
local strFontSmall = "ZSHUDFontSmall"
local txth
local strWaiting = translate.Get("waiting_for_players")
local strSpace = " "
local strHClosestToZ = translate.Get("humans_closest_to_spawns_are_zombies")
local desiredzombies
local strInitZombies = "number_of_initial_zombies_this_game"
local strZVolunteers = translate.Get("zombie_volunteers")
local strFontTiny = "ZSHUDFontTiny"
local y
local drown
local strBreath = translate.Get("breath")
local strCanPurchase = "PlayerCanPurchase"
local strPressF2ForShop = translate.Get("press_f2_for_the_points_shop")
local th
local strDiscountBetweenWaves = "x_discount_for_buying_between_waves"
function GM:HumanHUD(screenscale)
	curtime = CurTime()
	w, h = ScrW(), ScrH()

	packup = MySelf.PackUp
	...
	if not self.RoundEnded then
		if self:GetWave() == 0 and not self:GetWaveActive() then
			txth = draw_GetFontHeight(strFontSmall)
			draw_SimpleTextBlurry(strWaiting..strSpace..util.ToMinutesSeconds(math.max(0, self:GetWaveStart() - curtime)), strFontSmall, w * 0.5, h * 0.25, COLOR_GRAY, TEXT_ALIGN_CENTER)
			draw_SimpleTextBlurry(strHClosestToZ, strFontSmall, w * 0.5, h * 0.25 + txth, COLOR_GRAY, TEXT_ALIGN_CENTER)

			desiredzombies = self:GetDesiredStartingZombies()

			draw_SimpleTextBlurry(translate.Format(strInitZombies, self.WaveOneZombies * 100, desiredzombies), strFontSmall, w * 0.5, h * 0.75, COLOR_GRAY, TEXT_ALIGN_CENTER)

			draw_SimpleTextBlurry(strZVolunteers, strFontSmall, w * 0.5, h * 0.75 + txth, COLOR_GRAY, TEXT_ALIGN_CENTER)
			y = h * 0.75 + txth * 2

			txth = draw_GetFontHeight(strFontTiny)
			for i, pl in ipairs(self.ZombieVolunteers) do
				if pl:IsValid() then
					draw_SimpleTextBlurry(pl:Name(), strFontTiny, w * 0.5, y, pl == MySelf and COLOR_RED or COLOR_GRAY, TEXT_ALIGN_CENTER)
					y = y + txth
				end
			end
		end

		drown = MySelf.status_drown
		if drown and drown:IsValid() then
			...
			draw_SimpleTextBlurry(strBreath..strSpace, strFontSmall, w * 0.4, h * 0.35 + 6, COLOR_BLUE, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		end
	end

	if gamemode.Call(strCanPurchase, MySelf) then
		if self:GetWaveActive() then
			draw_SimpleTextBlurry(strPressF2ForShop, strFontSmall, w * 0.5, 8, COLOR_GRAY, TEXT_ALIGN_CENTER)
		else
			th = draw_GetFontHeight(strFontSmall)
			draw_SimpleTextBlurry(strPressF2ForShop, strFontSmall, w * 0.5, 8, COLOR_GRAY, TEXT_ALIGN_CENTER)
			draw_SimpleTextBlurry(translate.Format(strDiscountBetweenWaves, self.ArsenalCrateDiscountPercentage), strFontSmall, w * 0.5, 9 + th, COLOR_GRAY, TEXT_ALIGN_CENTER)
		end
	end
end
...
local screenscale
local myteam
local strClassicMode = "classicmode"
local strTClassicMode = translate.Get("classic_mode")
...
function GM:_HUDPaint()
	...
	if GetGlobalBool(strClassicMode) then
		draw_SimpleTextBlurry(strTClassicMode, strFontSmaller, 4, ScrH() - 4, COLOR_GRAY, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
	end
end
...
local obsmode
local strFontSmall = "ZSHUDFontSmall"
local strThatLife = translate.Get("that_life")
local strFont = "ZSHUDFont"
local strDmgToBarricades = "x_damage_to_barricades"
local strDmgToHumans = "x_damage_to_humans"
local strBrains = "x_brains_eaten"
local pl
local bossname
local strWaitingForWave = translate.Get("waiting_for_next_wave")
local strBeBossSoon = "you_will_be_x_soon"
local strXBeY = "x_will_be_y_soon"
local strBabyProp = "prop_thrownbaby"
local strPressToSpawn = "press_walk_to_spawn_as_x"
local strQ = "'"
local strGC = "Gore Child"
function GM:ZombieHUD()
	if self.LifeStatsEndTime and CurTime() < self.LifeStatsEndTime and (self.LifeStatsBarricadeDamage > 0 or self.LifeStatsHumanDamage > 0 or self.LifeStatsBrainsEaten > 0) then
		...
		draw_SimpleTextBlur(strThatLife, strFontSmall, x, y, colLifeStats, TEXT_ALIGN_LEFT)
		y = y + th

		if self.LifeStatsBarricadeDamage > 0 then
			draw_SimpleTextBlur(translate.Format(strDmgToBarricades, self.LifeStatsBarricadeDamage), strFontSmall, x, y, colLifeStats, TEXT_ALIGN_LEFT)
			y = y + th
		end
		if self.LifeStatsHumanDamage > 0 then
			draw_SimpleTextBlur(translate.Format(strDmgToHumans, self.LifeStatsHumanDamage), strFontSmall, x, y, colLifeStats, TEXT_ALIGN_LEFT)
			y = y + th
		end
		if self.LifeStatsBrainsEaten > 0 then
			draw_SimpleTextBlur(translate.Format(strBrains, self.LifeStatsBrainsEaten), strFontSmall, x, y, colLifeStats, TEXT_ALIGN_LEFT)
			y = y + th
		end
	end

	obsmode = MySelf:GetObserverMode()
	if obsmode ~= OBS_MODE_NONE then
		self:ZombieObserverHUD(obsmode)
	elseif not self:GetWaveActive() then
		th = draw_GetFontHeight(strFont)
		x = ScrW() * 0.5
		y = ScrH() * 0.15
		draw_SimpleTextBlur(strWaitingForWave, strFont, x, y, COLOR_DARKRED, TEXT_ALIGN_CENTER)
		if GAMEMODE.NextBossZombies then
			for i = 1, #GAMEMODE.NextBossZombies do
				pl = Entity(GAMEMODE.NextBossZombies[i][1])
				bossname = GAMEMODE.NextBossZombies[i][2]
				if pl and pl:IsValid() and bossname then
					if pl == MySelf then 
						draw_SimpleTextBlur(translate.Format(strBeBossSoon, strQ..bossname..strQ), strFont, x, y+th * i, COLOR_RED, TEXT_ALIGN_CENTER)
					else 
						draw_SimpleTextBlur(translate.Format(strXBeY, pl:Name(), strQ..bossname..strQ), strFont, x, y+th * i, COLOR_GRAY, TEXT_ALIGN_CENTER)
					end
				end
			end
		end
		if MySelf:GetZombieClassTable().NeverAlive then
			for _, ent in pairs(ents.FindByClass(strBabyProp)) do
				if ent:GetSettled() then
					draw_SimpleTextBlur(translate.Format(strPressToSpawn, self.ZombieClasses[strGC].Name), strFontSmall, w * 0.5, h * 0.75, COLOR_DARKRED, TEXT_ALIGN_CENTER)
					break
				end
			end
		end
...
function GM:_HUDShouldDraw(name)
	return not tShouldDraw[name]
end
--[[ -- -MagicSwap: I don't actually know if the table lookup method is faster than this but w/e
function GM:_HUDShouldDraw(name)
...
function GM:CreateVGUI()
	local screenscale = BetterScreenScale()
	local KFeedInt = GetConVar("zs_killfeed_enable"):GetInt()
	--laggy, disabled and replaced with a direct hud draw function
	--[[
	...
	if KFeedInt and KFeedInt == 1 then --disable more panel overhead, not making this direct hud draw but rather governed by a CVar
	...
	--this is necessary, could be made into direct draw
...
function GM:HumanMenu()
	...
	for k, v in pairs(self.AmmoNames) do
		if MySelf:GetAmmoCount(k) == 0 then continue end
...
function GM:_CalcView(pl, origin, angles, fov, znear, zfar)
	...
	if pl.Revive and pl.Revive:IsValid() and pl.Revive.GetRagdollEyes then
		rpos, rang = pl.Revive:GetRagdollEyes(pl)
		if rpos then
			origin = rpos
			angles = rang
		end
	elseif pl.KnockedDown and pl.KnockedDown:IsValid() then
		rpos, rang = self:GetRagdollEyes(pl)
		if rpos then
			origin = rpos
			angles = rang
		end
	...
	target = pl:GetObserverTarget()
	if target and target:IsValid() then
		lasttarget = self.LastObserverTarget
		if lasttarget and lasttarget:IsValid() and target ~= lasttarget then
			if self.LastObserverTargetLerp then
				if CurTime() >= self.LastObserverTargetLerp then
					...
					delta = math.Clamp((self.LastObserverTargetLerp - CurTime()) / 0.3333, 0, 1) ^ 0.5
...
function GM:PreDrawViewModel(vm, pl, wep)
	...
	if pl:IsHolding() then return true end
...
hook.Remove("PreDrawHalos", "CraftingHalo")
hook.Add("PreDrawHalos", "CraftingHalo", function()
	if GAMEMODE.CraftingEntity and GAMEMODE.CraftingEntity:IsValid() then
		local flash = math.abs(math.sin(RealTime() * 7)) * 6
		if GAMEMODE.HumanMenuPanel and GAMEMODE.HumanMenuPanel:Valid() and GAMEMODE.HumanMenuPanel:IsVisible() and MySelf:KeyDown(GAMEMODE.MenuKey) then
			halo.Add({GAMEMODE.CraftingEntity}, team.GetColor(TEAM_HUMAN), 2+flash, 2+flash, 2)
		else
			GAMEMODE.CraftingEntity = nil
		end
	end
end)
...
local function EndRoundCalcView(pl, origin, angles, fov, znear, zfar)
	...
	local music = GAMEMODE.LastHumanMusic
...
function GM:EndRound(winner, nextmap)
	...
	local music = self.LastHumanMusic
	--theres no clean way to bypass the stopsound without doing something hacky (stopping then restarting the stream)
	--so instead we simply lower the music volume if it exists then raise it later, stopsound is called if we're not using LH music 
	--perhaps there is a way to make it so stopsound doesn't affect the stream -MagicSwap
	if music and music:IsValid() then
		music:SetVolume(0.1)
	else
	...
	if snd and not (music and music:IsValid()) then
...
net.Receive("zs_nextboss", function(length)
	GAMEMODE.NextBossZombies = net.ReadDataTable(length / 8)
...
net.Receive("zs_boss_spawned", function(length)
	local tBosses = net.ReadDataTable(length / 8)
	if tBosses then
		for index, classindex in pairs(tBosses) do
...
local function PaintLHTrack()
	local name = GAMEMODE.LastHumanName
	local track = GAMEMODE.LastHumanTrackIndex

	if name and track and GAMEMODE.LHMusic[track] and GAMEMODE.LHMusic[track][1] then
		draw.SimpleText("Now playing "..name.."'s Last Human track!", "ZSHUDFont", ScrW() / 2, 0, team.GetColor(TEAM_HUMAN), TEXT_ALIGN_CENTER)
		draw.SimpleText(GAMEMODE.LHMusic[track][1], "ZSHUDFont", ScrW() / 2, 50, team.GetColor(TEAM_HUMAN), TEXT_ALIGN_CENTER)
	end

	if CurTime() >= (GAMEMODE.RemoveLHInfo or 0) then
		hook.Remove("HUDPaint", "ShowLHMusic")
	end
end
...
net.Receive("zs_lasthuman", function(length)
	...
	local music
	if track then --play our LH music, TODO: currently relies on default track being empty, FIX THIS
		GAMEMODE:PlayMusicIndex(track)
		GAMEMODE.LastHumanName = pl and pl:IsValid() and pl:ClippedName()
		GAMEMODE.LastHumanTrackIndex = track
		GAMEMODE.RemoveLHInfo = CurTime() + 20
		hook.Add("HUDPaint", "ShowLHMusic", PaintLHTrack)
	end
	GAMEMODE.LastHumanSound = snd
