hook.Add("ShouldDrawLocalPlayer", "HideZombiesBehindWalls", function(ply)
    if ply:Team() == TEAM_ZOMBIE then
        local startpos = ply:EyePos()
        local endpos = LocalPlayer():EyePos()
...
local MAX_PARTICLES = 1000 -- Set your maximum limit
local currentParticles = 0

hook.Add("Think", "ParticleLimiter", function()
    -- Get the current number of particles
    currentParticles = #ents.FindByClass("class_name") -- Replace 'class_name' with the class name of your particles

    -- If we're over the limit, remove some particles
    if currentParticles > MAX_PARTICLES then
        for i = 1, currentParticles - MAX_PARTICLES do
            local particle = ents.FindByClass("class_name")[i] -- Replace 'class_name' with the class name of your particles
