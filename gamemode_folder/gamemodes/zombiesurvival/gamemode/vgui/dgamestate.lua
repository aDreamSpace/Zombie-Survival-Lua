...
function PANEL:Text1Paint()
	local text
	local override = MySelf:IsValid() and GetGlobalString(strHUDOverride1..MySelf:Team(), strEmpty)
...
function PANEL:Text2Paint()
	if GAMEMODE:GetWave() <= 0 then
		...
		draw.SimpleText(translate.Format(zinvstr, util.ToMinutesSeconds(timeleft)), self.Font, 0, 0, col)
	elseif GAMEMODE:GetWaveActive() then
		local waveend = GAMEMODE:GetWaveEnd()
		if waveend ~= -1 then
			local timeleft = math.max(0, waveend - CurTime())
			draw.SimpleText(translate.Format(waveEnd, util.ToMinutesSeconds(timeleft)), self.Font, 0, 0, 10 < timeleft and COLOR_GRAY or Color(255, 0, 0, math.abs(math.sin(RealTime() * 8)) * 180 + 40))
		end
	else
		local wavestart = GAMEMODE:GetWaveStart()
		if wavestart ~= -1 then
			local timeleft = math.max(0, wavestart - CurTime())
			draw.SimpleText(translate.Format(nextwave, util.ToMinutesSeconds(timeleft)), self.Font, 0, 0, 10 < timeleft and COLOR_GRAY or Color(255, 0, 0, math.abs(math.sin(RealTime() * 8)) * 180 + 40))
		end
	end
...
function PANEL:Text3Paint()
	if MySelf:IsValid() then
		if MySelf:Team() == TEAM_UNDEAD then
			...
			if toredeem > 0 then
				draw.SimpleText(translate.Format(brainseaten, MySelf:Frags()..strSlash..toredeem), self.Font, 0, 0, COLOR_DARKRED)
			else
				draw.SimpleText(translate.Format(brainseaten, MySelf:Frags()), self.Font, 0, 0, COLOR_DARKRED)
			end
		else
			draw.SimpleText(translate.Format(points, MySelf:GetPoints()..strSlash..MySelf:Frags()), self.Font, 0, 0, COLOR_DARKRED)
		end
	end
