...
function meta:ResetLastBarricadeAttacker(attacker, dmginfo)
	if attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD then
		...
		if self:HumanNearby() then
			...
			PointCalc(attacker, dmg * 0.2, dmginfo:GetDamagePosition())
...
function meta:DamageNails(attacker, inflictor, damage, dmginfo)
	...
	if attacker:IsPlayer() then
		...
		--self.m_UndeadDamage = (self.m_UndeadDamage or 0) + damage
	end
...
function meta:RemoveNail(nail, dontremoveentity, removedby)
	...
	local nt = nail:GetNailType() or NAILTYPE_NONE
	local types = self:GetNailTypes()
	if not types[nt] then --no nails of this type remain, start checking if we need to reverse properties
		--TODO: find a way to do this modularly, typing this by hand is awful
		if nt == NAILTYPE_HEAVY and self.NailType == nt then --we're removing the last heavy nail
			self:SetBarricadeRepairs(self:GetBarricadeRepairs() / 1.25)
			self:SetMaterial(self._OLDMAT)
			self.NailType = nil
		end

		if nt == NAILTYPE_BARBED and self.NailType == nt then --we're removing the last barbed nail
			self:SetMaxBarricadeHealth(self:GetMaxBarricadeHealth() / 0.75)
			self:SetColor(self._OLDCOLOR)
			self.NailType = nil
		end

		if nt == NAILTYPE_GALVANIZED and self.NailType == nt then --we're removing the last galvanized nail
			self:SetBarricadeRepairs(self:GetBarricadeRepairs() / 1.1)
			self:SetMaxBarricadeHealth(self:GetMaxBarricadeHealth() / 1.15)
			self:SetBarricadeHealth(math.min(self:GetMaxBarricadeHealth(), self:GetBarricadeHealth() / 1.15))
			
			self:SetColor(self._OLDCOLOR)
			self.NailType = nil
		end

		if nt == NAILTYPE_STEEL and self.NailType == nt then --we're removing the last steel nail
			self:SetMaxBarricadeHealth(self:GetMaxBarricadeHealth() / 1.35)
			self:SetBarricadeHealth(math.min(self:GetMaxBarricadeHealth(), self:GetBarricadeHealth() / 1.35))
