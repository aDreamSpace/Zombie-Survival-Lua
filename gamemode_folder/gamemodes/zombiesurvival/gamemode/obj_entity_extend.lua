...
local E_GetDTBool = meta.GetDTBool
...
function meta:SetBarricadeRepairs(m)
	if m > self:GetMaxBarricadeRepairs() then
		m = self:GetMaxBarricadeRepairs()
	end
	self:SetDTFloat(3, m)
end
...
function meta:GetNailTypes()
	local temptbl = {}
	if self.Nails then
		for i, nail in pairs(self.Nails) do
			if nail.GetBaseEntity and nail:GetBaseEntity() == self then --ONLY count base entities!
				local nt = nail:GetNailType()
				if nt and nt ~= NAILTYPE_NONE then
					if not temptbl[nt] then
						temptbl[nt] = true
					end
				end
			end
...
function meta:GetMaxBarricadeRepairs()
	...
	local repairs = self:GetMaxBarricadeHealth() * 1.5
	if types[NAILTYPE_HEAVY] then
		return repairs * 2
	end
	return repairs
end
...
function meta:IsNailed() --caching this saves a ton of frames when phasing through barricades with many props
	if CLIENT and cache[self] ~= nil then
		return cache[self]
	end
...
timer.Create("NailTimer", 3, 0, function() --gotta refresh the cache to prevent mishaps
	if not stale then
		cache = {}
		stale = true
	end
