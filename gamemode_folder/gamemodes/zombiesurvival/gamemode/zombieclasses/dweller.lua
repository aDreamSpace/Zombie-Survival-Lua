...
function CLASS:OnSpawned(pl)
    ...
    timer.Create("DwellerDrain"..pl:EntIndex(), 1, 0, function()
        if not IsValid(pl) then
            timer.Remove("DwellerDrain"..pl:EntIndex())
            return
        end
        ...
        for _, ent in pairs(entities) do
            ...
            if ent:IsPlayer() and ent:Team() == TEAM_HUMAN then
                ...
                local newHealth = ent:Health() - 2

                -- Check if the new health is less than or equal to 0
                if newHealth <= 0 then
                    -- Kill the entity
                    ent:Kill()
                else
                    -- Set the entity's health to the new health
