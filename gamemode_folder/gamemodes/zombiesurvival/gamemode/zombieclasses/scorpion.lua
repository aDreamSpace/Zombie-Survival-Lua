...
hook.Add("EntityTakeDamage", "ScorpionDamageReduction", function(target, dmginfo)
    -- Check if the target is a player and is a Scorpio
    if target:IsPlayer() and target:Team() == TEAM_UNDEAD and target:GetZombieClassTable().Name == "Scorpio" then
        -- Reduce the damage by 51%
        dmginfo:ScaleDamage(0.49)
    end
end)
