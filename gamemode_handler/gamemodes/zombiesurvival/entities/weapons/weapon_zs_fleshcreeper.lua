...
local testmins = Vector(-16, -16, 0)
local testmaxs = Vector(16, 16, 72)
...
--[[
	...
	--]]
		--hulltrace using optimized physics calc for playerhull, optimized for error
	hitpos = tr.HitPos
	hitpos.z = hitpos.z + 4
	local htracedata = {}
	htracedata.start = hitpos
	htracedata.endpos = hitpos
	htracedata.filter = owner
	htracedata.mins = testmins
	htracedata.maxs = testmaxs
	htracedata.mask = MASK_PLAYERSOLID
	local trace = util.TraceHull(htracedata)
	
	if trace.HitWorld or trace.HitSky or (trace.Entity and trace.Entity:IsValid() and !trace.Entity:IsPlayer()) then
			self:SendMessage("not_enough_room_for_a_nest")
			return
	end
--[[
...
--]]
	...
	if ent:IsValid() then
		...
		ent.Owner = owner
		ent:SetOwnerName(owner:ClippedName())
