...
SWEP.CanLifeSteal = true

SWEP.ActivityTranslateSwing = {
    [ACT_HL2MP_IDLE] = ACT_HL2MP_IDLE_MELEE,
    [ACT_HL2MP_RUN] = ACT_HL2MP_RUN_MELEE,
    [ACT_HL2MP_IDLE_CROUCH] = ACT_HL2MP_IDLE_CROUCH_MELEE,
    ...
    [ACT_HL2MP_JUMP] = ACT_HL2MP_JUMP_MELEE,
...
function SWEP:CanPrimaryAttack()
    ...
    if owner:GetStamina() and self.StaminaUse then
        if owner:GetStamina() < self.StaminaUse then
...
function SWEP:MeleeSwing()
    ...
    if owner:GetStamina() and self.StaminaUse and IsFirstTimePredicted() then
        owner:RemoveStamina(self.StaminaUse)
    end
    ...
    if tr.Hit then
        ...
        if SERVER and hitent:IsValid() then
            ...
            if not hitent:IsPlayer() and self:GetOwner():Team() == TEAM_UNDEAD and self.MeleeDamageProp then
                damage = self.MeleeDamageProp
            end
            ...
            if hitent:GetClass() == "func_breakable_surf" then
                ...
                if hitent:IsPlayer() then
                    ...
                    if self.MeleeKnockBack > 0 then
                        ...
                        hitent:ThrowFromPositionSetZ(tr.HitPos, self.MeleeKnockBack * 3, nil, true)
                    end
