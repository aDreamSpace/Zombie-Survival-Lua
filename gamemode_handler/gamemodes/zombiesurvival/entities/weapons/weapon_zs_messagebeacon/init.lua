...
function SWEP:PrimaryAttack()
	...
	if ent:IsValid() then
		...
		--ent:SetMessageText(self.Msg or "Here!")
		ent.MsgSet = self.Msg or "Here!"
...
concommand.Add("setmessagebeacon", function(sender, cmd, args)
	if not sender:IsValid() then return end

	local wep = sender:GetActiveWeapon()
	
	if #args[1] > 32 then
		sender:ChatPrint("Cannot set message beacon. Message is too long! (max 32 characters)")
	end
	
	if wep:IsValid() and wep:GetClass() =="weapon_zs_messagebeacon" then
		wep.Msg = #args[1] <= 32 and args[1] or "Here!"
	end
	print(wep.Msg)
end)