...
local function DoFleshThrow(pl, wep)
	if pl:IsValid() and pl:Alive() and wep:IsValid() then
		...
		local amt = 10
		if pl:GetZTraitCurrent("pshower") then
			amt = amt + 8
		end
...
function SWEP:ModBaseDamage(dmg)
	if self.CADEBREAK then
		self.CADEBREAK = false
		return dmg * 2
	end
	return dmg
end
...
function SWEP:PrimaryAttack()
	...
	if owner:GetZTraitCurrent("cbreak") then
		trace.start = owner:GetShootPos()
		trace.endpos = owner:GetShootPos() + owner:GetAimVector() * self.MeleeReach
		trace.filter = owner:GetMeleeFilter()
		local tr = util.TraceLine(trace)
		local ent = tr.Entity
		if ent and ent:IsValid() and ent:IsBarricadeProp() then
			self.CADEBREAK = true
