...
function SWEP:SecondaryAttack()
	...
	if SERVER and !owner:KeyDown(IN_DUCK) then
		local ent = ents.Create("projectile_zsgrenade")
		if ent:IsValid() then
			ent:SetPos(owner:GetShootPos())
			ent:SetOwner(owner)
			...
			if phys:IsValid() then
				phys:Wake()
				phys:AddAngleVelocity(VectorRand() * 5)
				phys:SetVelocityInstantaneous(self.Owner:GetAimVector() * 400)
			end
		end
		
	elseif SERVER and owner:KeyDown(IN_DUCK) then
		local ent = ents.Create("projectile_zsgrenade")
		if ent:IsValid() then
			...
			if phys:IsValid() then
				phys:Wake()
				local RollAngle = self.Owner:GetAimVector()
				RollAngle.z = 0
				phys:SetVelocityInstantaneous(RollAngle * 700)
