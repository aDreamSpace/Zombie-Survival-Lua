...
local function RefreshViewModelBoneMods()
	...
	timer.Create("repop", 1, 1, function()
		local option = PopulateBoneList( wep.v_modelbonebox, MySelf:GetViewModel() )
		if (option) then
...
local vhbox = vgui.Create( "DCheckBoxLabel", pweapon )
	...
	vhbox.OnChange = function()
		...
		if (wep.ShowViewModel) then
			...
			MySelf:GetViewModel():SetMaterial("")
		else
			MySelf:GetViewModel():SetColor(Color(255,0,0,255))
			...
			MySelf:GetViewModel():SetMaterial("Debug/hsv") 
		end
	end
...
timer.Simple(1, function()
	local option = PopulateBoneList( vsbonebox, MySelf:GetViewModel() )
...
local wpcbtn = vgui.Create( "DButton", pweapon )
	...
	wpcbtn.DoClick = function()
		...
		MySelf:ChatPrint("Code copied to clipboard!")
	end
...
local wpbtn = vgui.Create( "DButton", pweapon )
	...
	wpbtn.DoClick = function()
		...
		MsgN("*********************************************")
		MySelf:ChatPrint("Code printed to console!")
	end
