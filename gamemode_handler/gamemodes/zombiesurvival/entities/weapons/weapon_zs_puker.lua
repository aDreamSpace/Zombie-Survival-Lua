...
function SWEP:ShootBullets(dmg, numbul, cone)
    ...
    bullet.Callback = function(attacker, tr, dmginfo)
        -- Check if the distance between the attacker and the hit position is greater than the range limit
        if attacker:GetPos():Distance(tr.HitPos) > 500 then
            return -- If it is, don't apply the damage
        end
...
function SWEP.BulletCallback(attacker, tr, dmginfo)
    -- Check if the distance between the attacker and the hit position is greater than the range limit
    if attacker:GetPos():Distance(tr.HitPos) > 500 then
        dmginfo:SetDamage(0) -- If it is, set the damage to 0
        return
    end
