...
function ENT:OnDamaged(damage, attacker, inflictor, dmginfo)
	...
	if attacker:IsValid() and attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD and ent:IsValid() then
		ent.m_ZombieHitAmount = (ent.m_ZombieHitAmount or 0) + damage
		self:SetZombieHitAmount(self:GetZombieHitAmount() or 0 + damage)
	end
end
...
function ENT:AttachTo(baseent, attachent, physbone, physbone2)
	...
	--local types = baseent:GetNailTypes() --cache here so we dont include this nail
	...
	--fixup
	baseent:SetBarricadeRepairs(baseent:GetBarricadeRepairs())

	--if we're adding a heavy nail, we should increase the hp, but only if this is the first nail
	--later on we decrease it by a similar factor on remove!
	...
	if not baseent.NailType then
		...
		if nt == NAILTYPE_BARBED then
			...
			baseent:SetColor(Color(255, 0, 0))
