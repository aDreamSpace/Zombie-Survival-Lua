...
function ENT:SetupDataTables()
	self:NetworkVar("Float", 0, "ZombieHitAmount")
	self:NetworkVar("Int", 0, "NailType")
end
...
function ENT:SetAttachEntity(ent, physbone1, physbone2)
	...
	timer.Simple(0.1, function()
		if SERVER then
			local ply = self and self:IsValid() and self:GetDeployer()
			if ply and ply:IsValid() then
				if ent:IsValid() then
					ulx.logString("[CADE] Nail placed Prop2Prop by "..ply:Nick(), true)
				end
			end
		end 
		GAMEMODE:EvaluatePropFreeze() 
	end)
