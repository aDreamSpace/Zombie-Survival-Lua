...
function ENT:DrawTranslucent()
	if not self.setup then
		if weapons.GetStored(self:GetWeaponType()).PrintName then
			self.WepType = (weapons.GetStored(self:GetWeaponType()).PrintName)
		else
			self.WepType = "Weapon"
		end
	...
	if MySelf:Team() == TEAM_HUMAN then
		...
		if MySelf:GetPos():DistToSqr(vPos) < self.displayDistance then
			...
			if EyeVector():Dot(vB) > 0.85 then
				...
				local bSnakeEyes = MySelf.m_SnakeEyes
				cam.Start3D2D(vPos + self.upOffset, aEyeAng, 0.05)
					...
					draw.ShadowText((ammonum > 0 and strClip..ammonum or strNone) or (ammonum == 0 and strNone) or (ammonum < 0 and strNo), ammoText, 0, self.vertFontSize + self.halfVertFontSize * 5, white, black, 2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
