function GM:CraftItem(pl, recipe, enta, entb)
	...
	if not recipe.OnCraft or not recipe.OnCraft(pl, enta, entb) then
		...
		if result then
			...
			if string.sub(resultclass, 1, 7) == "weapon_" or string.StartWith(resultclass, "cw_") then
				if pl:HasWeapon(resultclass) then
					...
					if weptab and weptab.AmmoIfHas then
						...
						timer.Simple(0, function()
							pl:SetActiveWeapon(resultclass)
						end)
					end
				end
			elseif result[2] and result[2] == "CW_ATT" then --extra case for cw attachment crafting
				CustomizableWeaponry:giveAttachment(pl, result[1], false)
			else
				local target = self:GetCraftTarget(enta, entb)
			...
			PushAchievementProgress(pl, "Crafter")
		end
...
concommand.Add("_zs_craftcombine", function(sender, command, arguments)
	...
	local entbArtificial
	local class
	if entb:IsWeapon() then
		class = entb:GetClass()
		entb = ents.Create("prop_weapon")
		if entb and entb:IsValid() then
			entb:SetWeaponType(class)
			entb:SetPos(sender:GetPos() + Vector(0,0,32))
		end
		entbArtificial = true
	end
	...
	if recipe and gamemode.Call("CanCraft", sender, enta, entb) then
		...
		if class then sender:StripWeapon(class) end
	end
	if entbArtificial then entb:Remove() end
end)
