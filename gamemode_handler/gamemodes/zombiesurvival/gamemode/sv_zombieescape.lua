...
local strFilter = "filter_activator_team"
local strFT = "filterteam"
hook.Add("InitPostEntityMap", "zombieescape", function(fromze)
	local tEnts = ents.GetAll()
	for i = 1, #tEnts do
		ent = tEnts[i]
		if ent and ent:IsValid() then
			if ent.ZEFix and ent:GetClass() == strFilter then
				ent:SetKeyValue(strFT, ent.ZEFix)
			end
...
hook.Add("InitPostEntity", "EnableAttachments", function()
	timer.Simple(3, function() RunConsoleCommand("cw_enable_all_attachments_on_spawn") end)
end)
