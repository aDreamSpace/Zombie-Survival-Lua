...
function CLASS:PlayFootstepSound(pl)
    if GetConVarNumber("mute_advisor_footsteps") == 1 and pl:Alive() then
        pl:EmitSound("playermodel_hunter2s.footsteps")
        return true
    end
end
...
function CLASS:PlayDeathSound(pl)
    if GetConVarNumber("player_advisor_sounds") == 1 then
        pl:EmitSound("playermodel_hunter2s.die")
        return true
    end
end
...
function CLASS:PlayPainSound(pl)
    if GetConVarNumber("player_advisor_sounds") == 1 then
        pl:EmitSound("playermodel_hunter2s.pain")
        return true
    end
end
...
function CLASS:PostPlayerDraw(pl)
    ...
    for i = 1, numParticles do
        ...
        if particle then
            ...
            particle:SetStartSize(1) -- Set the size to 10
            particle:SetEndSize(0)
            particle:SetColor(0, 252, 210)-- Set the color to red
        end
    end
--Color(0, 252, 210)
    -- Release the emitter
...
if CLIENT then
    hook.Add("CalcView", "HunterThirdPersonView", function(ply, pos, angles, fov)
        if ply:Alive() and ply:GetZombieClassTable().Name == "Hunter" then
            local view = {}
