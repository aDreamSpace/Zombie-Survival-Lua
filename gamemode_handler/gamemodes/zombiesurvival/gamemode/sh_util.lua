...
--note: takes a squared radius
local tLOS = {nil, nil, nil, mask = MASK_SHOT_OPAQUE}
function util.BlastDamageHZ(inflictor, attacker, origin, radius, dmg, dmgtype)
	for _, ent in pairs(ents.GetAll()) do
		if ent:IsValid() and origin:DistToSqr(ent:GetPos()) <= radius then
			if ent:IsPlayer() then
				local nearest = ent:NearestPoint(origin)
				tLOS.start = origin
				tLOS.endpos = nearest
				tLOS.filter = player.GetAll()
				local tbl = tLOS.filter
				tbl[#tbl+1] = ent
				tbl[#tbl+1] = inflictor
				
				local tr = util.TraceLine(tLOS)
				if not tr.Hit then
					if ent:Team() == TEAM_HUMAN then
						ent:TakeSpecialDamage(((radius - nearest:DistToSqr(origin)) / radius * (dmg / 10)), dmgtype, attacker, inflictor, nearest)
					else
						ent:TakeSpecialDamage(((radius - nearest:DistToSqr(origin)) / radius * (dmg)), dmgtype, attacker, inflictor, nearest)
					end
				end
			end
		end
