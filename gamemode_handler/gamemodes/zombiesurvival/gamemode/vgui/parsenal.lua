--progress
--menu generates
--some nox code (ZSInventory) ripped out
--guns fail to generate
--CW Melee doesn't generate properly
--attachments dont generate
...
local function CanBuy(item, pan)
	...
	local cost = item.Price or item.Worth or item.price or 8 --hacky fix

	if MySelf:GetPoints() < math.floor(cost * (MySelf.ArsenalDiscount or 1)) then
		return false
	end
...
local function ItemPanelThink(self)
	local itemtab = FindItem(self.ID) or CustomizableWeaponry.registeredAttachmentsSKey[self.ID]
	if itemtab then
...
local function ItemPanelPaint(self, w, h)
	...
	if self.ShopTabl.SWEP and MySelf:HasWeapon(self.ShopTabl.SWEP) or MySelf.CWAttachments[self.ID] then
...
function GM:ViewerStatBarUpdate(viewer, display, sweptable)
	...
	for i = 1, 6 do
		...
		elseif statshow[1] == "ClipSize" then
			stattext = statnum / sweptable.AmmoPerShot or 1
		else
			stattext = statnum
		end
		...
		if statshow[1] == "Damage" then
			statnum = statnum * sweptable.Shots
		elseif statshow[1] == "ClipSize" then
			statnum = statnum / sweptable.AmmoPerShot or 1
		end
...
function GM:HasPurchaseableAmmo(sweptable)
	if sweptable.Primary and self.PointShopAmmoNames[sweptable.Primary.Ammo] then
		return true
	end
end
...
local function ItemPanelDoClick(self)
	...
	if MySelf.CWAttachments[self.ID] then return end
	if not shoptbl then return end
	local sweptable = weapons.Get(shoptbl.SWEP)

	if not sweptable or GAMEMODE.AlwaysQuickBuy then
		RunConsoleCommand("zs_pointsshopbuy", self.ID, self.AttID)
		return
	end
	...
	purb.ID = self.ID
	purb.DoClick = function() RunConsoleCommand("zs_pointsshopbuy", self.ID, self.AttID) end
	...
	local price = math.ceil(shoptbl.Worth * (not GAMEMODE:GetWaveActive() and GAMEMODE.ArsenalCrateMultiplier or 1))
	...
	purb = viewer.m_AmmoB
	if canammo then
		purb.AmmoType = GAMEMODE.PointShopAmmoNames[sweptable.Primary.Ammo]
...
function GM:AddShopItem(list, i, tab, issub, nopointshop)
	...
	local icon = cat ~= ITEMCAT_TRINKETS or cat ~= ITEMCAT_ATT
	...
	itempan:SetText("")
	itempan:SetSize(wid * screenscale, (icon and 100 or 60) * screenscale)
	itempan.ID = tab.Signature or i
	itempan.AttID = tab.name
	...
	itempan.DoRightClick = function()
		...
		menu:AddOption("Buy", function() RunConsoleCommand("zs_pointsshopbuy", itempan.ID, itempan.AttID) end)
		menu:Open()
	end
	...
	if icon then
		...
		local kitbl = killicon.Get(tab.SWEP or tab.Model or "unknown")
		if kitbl then
	...
	local name = tab.displayNameShort or tab.Name or ""
	local namelab = EasyLabel(itempan, name, "ZSHUDFontSmaller", COLOR_WHITE)
	namelab:SetPos(12 * screenscale, itempan:GetTall() * (icon and 0.8 or 0.7) - namelab:GetTall() * 0.5)
	if missing_skill then
		namelab:SetAlpha(30)
	end
	...
	if missing_skill then
		...
		local iprice = tab.Price or tab.Worth or tab.price or 10

		local points = math.floor(iprice * (MySelf.ArsenalDiscount or 1))
		local price = tostring(points)
		if nopointshop then
			price = tostring(math.ceil(self:PointsToScrap(price)))
		end
	...
	if not icon and tab.SubCategory then
		local strSubcat = GAMEMODE.ItemSubCategoriesEx[tab.Category][tab.SubCategory]
		local catlabel = EasyLabel(itempan, strSubcat, "ZSBodyTextFont")
...
function GM:OpenArsenalMenu()
	--if self.ArsenalInterface and self.ArsenalInterface:IsValid() then self.ArsenalInterface:Remove() end
	if self.ArsenalInterface and self.ArsenalInterface:IsValid() then
		...
		if self.RebuildAtts then
			self:RebuildAtts(MySelf:GetActiveWeapon() or weapons.GetStored("cw_melee_fists"))
		end
		return
	end
	...
	for catid, catname in SortedPairs(GAMEMODE.ItemCategories) do
		local hasitems = false
		for i, tab in ipairs(GAMEMODE.Items) do
			if tab.Category == catid and tab.PointShop or catid == ITEMCAT_ATT then
				hasitems = true
				break
			end
		end
		...
		if hasitems then
			...
			local attoffset = catid == ITEMCAT_ATT
			local offset = 64 * screenscale

			local itemframe = vgui.Create("DScrollPanel", tabpane)
			itemframe:SetSize(propertysheet:GetWide(), propertysheet:GetTall() - ((usecats or attoffset) and (32 + offset) or 32))
			itemframe:SetPos(0, (usecats or attoffset) and offset or 0)
			...
			local curcat = GAMEMODE.ItemSubCategoriesEx[catid]
			print(ITEMCAT_ATT, catid)
			if usecats then
				local ind, tbn = 1
				local curoffset = 0
				local before
				print(#curcat)
				for i = ind, #curcat do
					local ispacer = i --trinkets and  or i --???
					local start = i == ind--(catid == ITEMCAT_GUNS and 2 or ind) --why do we need this?
					tbn = EasyButton(tabpane, curcat[i], 2, 8)
					tbn:SetFont("ZSHUDFontSmallest")
					tbn:SetAlpha(start and 255 or 70)

					tbn:AlignTop(0) --trinkets and i <= 3 and 0 or trinkets and 28 or 16) --overcomplicated???
					tbn:SetContentAlignment(5)
					tbn:SizeToContents()
					curoffset = curoffset + (before and before:GetWide() or 0)
					before = tbn
					tbn:AlignLeft(curoffset)
			...
			elseif catid == ITEMCAT_ATT then
				--in order for this to work cleanly, we must do the following:
				--build the att panel as normal, populating the menu with current attachments
				--build a wrapper function that will refresh the entire attmenu sheet
				--call the wrapper function on menu reopened
				--as for load order with attachments, that can be relegated to the wrapper function or a call somewhere else

				self.AttPane = tabpane
				...
				self.RebuildAtts = function(me, wep)
					--[
					print(wep)
					local tabpane = me.AttPane
					for k, v in pairs(tabpane.Grids) do
					...
					for k, v in pairs(tabpane.Buttons) do
						if v and v:IsValid() then
							v:Remove()
						end
						k = nil
					...
					local tAllAtts = CustomizableWeaponry.registeredAttachmentsSKey
					local tAtts = wep.Attachments or {}
					local tDependencies = wep.AttachmentDependencies --theres more to this but w/e -MagicSwap
					local tAttSetup = {}

					local i = 1
					local before
					local curoffset = 0
					for icur, tInfo in pairs(tAtts) do
						local strCat = tInfo.header or ""
						local ispacer = i
						local start = i == 1

						local tbn = EasyButton(tabpane, strCat, 2, 8)
						tbn:SetFont("ZSHUDFontSmallest")
						tbn:SetAlpha(start and 255 or 70)
						curoffset = curoffset + (before and before:GetWide() or 0)
						before = tbn

						tabpane.Grids[icur] = mkgrid()
						tabpane.Grids[icur]:SetVisible(start)
						tabpane.Buttons[icur] = tbn
						...
						tbn.DoClick = function(me)
							for k, v in pairs(tabpane.Grids) do
								print(v)
								v:SetVisible(k == icur)
								tabpane.Buttons[k]:SetAlpha(k == icur and 255 or 70)
							end
						end

						--tAttSetup[i] = tInfo.atts
						for _, strAtt in pairs(tInfo.atts) do
							local tAtt = tAllAtts[strAtt]
							if tAtt then
								me:AddShopItem(tabpane.Grids[icur], tAtt.name, tAtt)
							end
						end
			...
			for i, tab in ipairs(GAMEMODE.Items) do
				if tab.PointShop and tab.Category == catid then
					self:AddShopItem(
						curcat and tabpane.Grids[tab.SubCategory] or tabpane.Grid or tabpane.Grids[tab.Tier or 1],
						i, tab
					)
				end
			end
			if catid == ITEMCAT_ATT then 
				self:RebuildAtts(MySelf:GetActiveWeapon() or weapons.GetStored("cw_melee_fists"))
			end
...
function GM:OpenPointsShop() self:OpenArsenalMenu() end --legacy