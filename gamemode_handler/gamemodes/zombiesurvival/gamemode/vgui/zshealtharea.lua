...
local function ContentsPaint(self)
	...
	if lp:IsValid() then
		...
		draw.SimpleTextBlurry(health, strFont, 8, self:GetTall() - 8, colHealth, TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
	end
end
...
function PANEL:Init()
	...
	local lifestatus = vgui.Create("ZSHealthStatus", contents)
	lifestatus:SetTall(20)
	lifestatus:SetAlpha(200)
	lifestatus:SetColor(Color(0, 255, 0))
	lifestatus:SetMemberName("HEAL!")
	lifestatus.GetMemberValue = function(me)
		local lp = MySelf
		if lp:GetRegenLife() > 0 then
			return lp:GetRegenLife()
		end

		return 0
	end
	lifestatus.MemberMaxValue = 100
	lifestatus:Dock(TOP)

	local burnstatus = vgui.Create("ZSHealthStatus", contents)
	burnstatus:SetTall(20)
	burnstatus:SetAlpha(200)
	burnstatus:SetColor(Color(255, 125, 0))
	burnstatus:SetMemberName("BURN!")
	burnstatus.GetMemberValue = function(me)
		local lp = MySelf
		if lp:GetFireDamage() > 0 then
			return lp:GetFireDamage()
		end

		return 0
	end
	burnstatus.MemberMaxValue = 250
	burnstatus:Dock(TOP)
...
function PANEL:Paint()
	...
	draw.SimpleText(t1, strFont, w - 3, h / 2 + 1, color_black, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	draw.SimpleText(t1, strFont, w - 4, h / 2, color_white, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	draw.SimpleText(self.MemberName, strFont, 5, h / 2 + 1, color_black, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	draw.SimpleText(self.MemberName, strFont, 4, h / 2, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
end
