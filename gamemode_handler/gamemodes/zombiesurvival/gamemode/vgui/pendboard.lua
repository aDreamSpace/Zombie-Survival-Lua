...
hook.Add("OnPlayerChat", "BringRoundResults", function(ply, text, team)
	if text:lower() == "!results" then
		if ply:IsValid() and ply == MySelf then
			if pEndBoard and pEndBoard:IsValid() then
				pEndBoard:SetVisible(true)
			else
				chat.AddText(Color(255, 0, 0), "End board is not available at this time.")
			end
		end
	end
end)
...
function MakepEndBoard(winner)
	...
	elseif winner == TEAM_UNDEAD then
		...
		heading = EasyLabel(frame, "The Undead have won!", "ZSHUDFont", COLOR_GREEN)
	else
		surface.PlaySound("ambient/levels/citadel/strange_talk"..math.random(3, 11)..".wav")
