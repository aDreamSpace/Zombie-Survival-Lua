...
if engine.ActiveGamemode() == "zombiesurvival" and false then --disabled for now...
	hook.Add("PlayerSpawn", "!human info", function(pl)
		if game.GetMap():StartWith("zs_obj") then return end --no redeeming on obj
	...
	function ulx.giveHumanLoadout(pl)
		...
		pl:Give("cw_pist_makarov")
		pl:GiveAmmo(260, "pistol")
	end
	...
	function ulx.human(pl)
		...
		if game.GetMap():StartWith("zs_obj") then
			local response = "Self-redeeming is disabled on Objective maps."
			pl:ChatPrint(response)
			pl:PrintMessage(HUD_PRINTCENTER, response)
			return
		end
...
registerAdminCmd("BotMod", numParam, function(caller, num)
	...
	ulx.fancyLogAdmin(caller, "#A has changed botmod formula to "..num.."!")
...
registerSuperadminCmd("Finish", function(caller) if SERVER then cookie.Set(game.GetMap().."Mesh", "1") end caller:ChatPrint("Map navmesh marked as final.") end)
registerSuperadminCmd("UnFinish", function(caller) if SERVER then cookie.Set(game.GetMap().."Mesh", "0") end caller:ChatPrint("Map navmesh marked as incomplete.") end)
