local att = {}
att.name = "kry_hk33_sg1"
att.displayName = "Hensoldt Wetzlar ZF-1 4x24"
att.displayNameShort = "ZF1 4x24"
att.aimPos = {"ZFSG1Pos", "ZFSG1Ang"}
att.FOVModifier = 15
att.isSight = true
...
if CLIENT then
	att.displayIcon = surface.GetTextureID("atts/kry_hk33_sg1")
	...
	local old, x, y, ang
	local reticle = surface.GetTextureID("cw2/reticles/kry_hk_g3_sg1_reticle")
	...
	function att:drawRenderTarget()
		...
		if not self.freeAimOn then
			ang:RotateAroundAxis(ang:Right(), self.ZFSG1AxisAlign.right)
			ang:RotateAroundAxis(ang:Up(), self.ZFSG1AxisAlign.up)
			ang:RotateAroundAxis(ang:Forward(), self.ZFSG1AxisAlign.forward)
		end
		...
		render.SetViewPort(0, 0, size, size)
			...
			cam.Start2D()
				...
				surface.DrawTexturedRectRotated(size * 0.5, size * 0.5, size, size, 180)
