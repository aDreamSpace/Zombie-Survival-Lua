...
function meta:D3bot_GetAttackPosOrNil(fraction, target)
    ...
    if mem.attackPosCache and mem.attackPosCache.tgt == tgt and mem.attackPosCache.fraction == fraction then
        return mem.attackPosCache.result
    end

    -- No cached result, so calculate it
    local result = tgt:IsPlayer() and LerpVector(fraction or 0.75, tgt:GetPos(), tgt:EyePos()) or tgt:WorldSpaceCenter()
