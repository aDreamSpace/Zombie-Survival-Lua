...
function Player:PS_PlayerSpawn()
    if self:Team() == TEAM_HUMAN and self.PS_Items then
        for id, item in pairs(self.PS_Items) do
            if item.Equip and PS.Items[id].Function and PS.Items[id].Function.PlayerSpawn then
                PS.Items[id].Function.PlayerSpawn(self, id)
            end
        end
    end
end
...
function Player:PS_TakeItem(item_id)
    -- Ensure that the PS_Items table is initialized
    self.PS_Items = self.PS_Items or {}
