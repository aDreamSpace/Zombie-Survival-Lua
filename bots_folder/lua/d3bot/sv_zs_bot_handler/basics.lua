...
-- Global variable to keep track of the number of attacking bots
attackingBotsCount = 0
local MAX_ATTACKING_BOTS = 80 -- Set your limit here
local lastTrajectoryComputeTime = 0
local TRAJECTORY_COMPUTE_INTERVAL = 25 -- Compute the trajectory every 1 second

-- Find best trajectory
local trajectory
if CurTime() - lastTrajectoryComputeTime > TRAJECTORY_COMPUTE_INTERVAL then
    ...
    lastTrajectoryComputeTime = CurTime()
end
...
if (trajectory and CurTime() >= weapon:GetNextPrimaryFire() and CurTime() >= weapon:GetNextSecondaryFire() and CurTime() >= weapon.NextAllowPounce) or mem.pouncing then
    ...
    if not mem.pouncing then
        -- Check if the bot can start attacking
        if attackingBotsCount < MAX_ATTACKING_BOTS then
            -- Started pouncing
            actions.Attack2 = true
            mem.pouncingTimer = CurTime() + 0.9 + math.random() * 0.2
            mem.pouncingStartTime = CurTime() + weapon.PounceStartDelay
            mem.pouncing = true
            attackingBotsCount = attackingBotsCount + 1 -- Increment the count
        end
    elseif mem.pouncingTimer and mem.pouncingTimer < CurTime() and (CurTime() - mem.pouncingTimer > 5 or bot:WaterLevel() >= 2 or bot:IsOnGround()) then
        ...
        attackingBotsCount = attackingBotsCount - 1 -- Decrement the count
    end
