...
local cvMuzzle = GetConVar("cw_muzzleflash")
local cvLighting = GetConVar("cw_muzzlelighting")
function SWEP:CreateMuzzle()
	if self.Owner:ShouldDrawLocalPlayer() then
		return
	end
	local MuzEffect = cvMuzzle:GetInt()
		...
		if muz then
			...
			if muz2 then
				...
				if self.dt.Suppressed then
					...
					local MuzFlash = cvLighting:GetInt()
...
function SWEP:applyOffsetToVM()
	...
	if self.freeAimOn and (self.freeAimOn and not self.dt.BipodDeployed) then
		...
		fovDiff = math.Clamp(60 / (self.ViewModelFOV - self.BreathFOVModifier * 0.5), -math.huge, math.huge)
		...
		pitchDiff = math.AngleDifference(eyeAngles.p, actualEyeAngles.p) * 0.45 * fovDiff
		yawDiff = math.AngleDifference(eyeAngles.y, actualEyeAngles.y) * 0.45 * fovDiff
		
		-- normalize the angles after subtracting the difference in both axes
		normalizedPitch = math.NormalizeAngle(eyeAngles.p - pitchDiff)
		normalizedYaw = math.NormalizeAngle(eyeAngles.y - yawDiff)
	...
	swayIntensity = self.dt.State == CW_AIMING and self.AimSwayIntensity or self.SwayIntensity
	...
	if self._RIFNADEATTACHTIME and self._RIFNADEATTACHTIME > CurTime() then
		RotateAroundAxis(ang, Right(ang), -12)
	end
	self.CW_VM:SetPos(pos)
	self.CW_VM:SetAngles(ang)
end

local cvLight = CreateConVar("cw_muzlight", 1, FCVAR_ARCHIVE, "Enables/disables the projected texture muzzleflash.")
local colMuz = Color(244, 158, 66)
local mat = "effects/flashlight/hard.vtf"
function SWEP:drawMuzzleLight()
	if cvLight:GetInt() ~= 0 and not self.CW20Melee then
		local MySelf = self:GetOwner()
		if MySelf and MySelf:IsValid() then
			local light = MySelf.m_WeaponLight
			local pos = EyePos()
			local ang = EyeAngles()
			if not (light and light:IsValid()) then
				MySelf.m_WeaponLight = ProjectedTexture()
				light = MySelf.m_WeaponLight
				light:SetTexture("effects/flashlight/soft")
				light:SetFOV(90)
				light:SetColor(colMuz)
			...
			local show = MySelf.m_WeaponLightTime
			local col = render.GetLightColor(pos)
			local avg = (col.x + col.y + col.z) / 3
			if show and show > RealTime() then
				local rand = math.Rand(-2, 2)
				local bright = avg > 0.3 and 1 or (4 + rand)
...
local POS
local cv3DHUD = GetConVar("gc_3dhud_ammo")
function SWEP:_drawViewModel()
	...
	if cv3DHUD:GetInt() > 0 then
		self:draw3D2DHUD()
	end

	self:drawMuzzleLight()
end
...
function SWEP:getReserveAmmoText()
	...
	if shouldOverride then
  	...
  	local ammoAmt = self.Owner:GetAmmoCount(self.Primary.Ammo)
  
  	if self.AmmoPerShot and self.AmmoPerShot > 1 then
    	ammoAmt = math.floor(ammoAmt / self.AmmoPerShot)
    end
  
	return ammoAmt, shouldOverride, targetColor
end

local zsammotbl = GAMEMODE.AmmoNames
local att
local ang
local FT
local mag
local reloadProgress
local strFont60 = "CW_HUD60"
local strReload = "RELOADING "
local strPC = "%"
local strSlash = " / "
local bulletDisplayAlpha
local bulletDisplayOffset
local aiming
local zsammo
local binding
local strFont48 = "CW_HUD48"
local strBind = "+menu"
local strNoBind = "not bound"
local strCanPurchase = "PlayerCanPurchase"
local strPress = "Press "
local strPurchase = " to purchase ammo"
local grenades
local curGrenade
local strNade = "Frag grenades"
local strFrag = "x FRAG"
local strFont40 = "CW_HUD40"
local strM203Ready = "M203 READY"
local strM203Empty = "M203 EMPTY"
local str40MM = "40MM"
local strReserve = "x RESERVE"
local strFont32 = "CW_HUD32"
local strType = "TYPE"
local strDraw = "drawTo3D2DHUD"
local bCanPurchase = false

function SWEP:draw3D2DHUD()
	att = self:getMuzzlePosition()
	...
	cam.Start3D2D(att.Pos + ang:Forward() * 4, ang, self.HUD_3D2DScale)
		cam.IgnoreZ(true)
			...
			if reloadProgress then
				draw.ShadowText(strReload .. reloadProgress .. strPC, strFont60, 90, 50, self.HUD_3D2D_MagColor, self.HUDColors.black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			else
				draw.ShadowText(self:getMagCapacity() .. strSlash .. self:getReserveAmmoText(), strFont60, 90, 50, self.HUD_3D2D_MagColor, self.HUDColors.black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			end
			
			if self.BulletDisplay and self.BulletDisplay > 0 then
				bulletDisplayAlpha = self.HUD_3D2DAlpha
				bulletDisplayOffset = 0
				...
				if #self.FireModes > 1 then -- if we have more than 1 firemode for the current weapon, we don't let the firemode display fade and instead reposition it a bit to let the player see what firemode he's using while aiming
					aiming = self.dt.State == CW_AIMING
			...
			zsammo = self.Primary.Ammo
			
			draw.ShadowText(zsammotbl[zsammo] or self.Primary.Ammo, strFont48, 90, 100, self.HUDColors.white, self.HUDColors.black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)

			binding = binding or (input.LookupBinding(strBind) or strNoBind):upper()
			
			if gamemode.Call(strCanPurchase, MySelf) then
				draw.ShadowText(strPress..binding..strPurchase, strFont48, 90, 145, self.HUDColors.white, self.HUDColors.black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			end

			grenades = self.Owner:GetAmmoCount(strNade)
			
			if grenades > 0 then
				draw.ShadowText(grenades .. strFrag, strFont40, 90, 140, self.HUDColors.white, self.HUDColors.black, 2, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
			end
			...
			if self.dt.M203Active then
				...
				if (not self.M203Chamber and self.dt.State ~= CW_AIMING) or self.M203Chamber then
					if not self.M203Chamber then
						draw.ShadowText(strM203Empty, strFont40, 90, -70, self.HUDColors.red, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
					else
						draw.ShadowText(strM203Ready, strFont40, 90, -70, self.HUDColors.white, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
					end
					
					curGrenade = CustomizableWeaponry.grenadeTypes.getGrenadeText(self)
					
					draw.ShadowText(self.Owner:GetAmmoCount(str40MM) .. strReserve, strFont32, 90, -40, self.HUDColors.white, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
					draw.ShadowText(strType .. curGrenade, strFont32, 90, -10, self.HUDColors.white, self.HUDColors.black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
				end
			end
			
			CustomizableWeaponry.callbacks.processCategory(self, strDraw)
		cam.IgnoreZ(false)
	cam.End3D2D()
end
...
function SWEP:drawInteractionMenu()
	...
	if self.CustomizeMenuAlpha > 0 then
		...
		if not att then owner = self:GetOwner() att = owner:EyePos() + owner:GetAimVector() * 24 end
		local ang = EyeAngles()
		ang:RotateAroundAxis(ang:Right(), 90)
		ang:RotateAroundAxis(ang:Up(), -90)
		ang:RotateAroundAxis(ang:Right(), -10)
		local pos = att.Pos or att
...
function SWEP:performViewmodelMovement()
	...
	if not self.IsReloading then
		self.IsReloading = self.IsReloadingOverride or false
	end
