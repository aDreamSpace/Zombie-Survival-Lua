...
function SWEP:attemptToggleInteractionMenu()
	...
	if self:canOpenInteractionMenu() then
		...
		if self.SpecialCAnimation and self:canCustomize() then
			local tbl = self.CustomizationAnims
			if self.dt.State == CW_CUSTOMIZE then
				self:playAnim(tbl.from[1], tbl.from[2], 0, nil, tbl.from[1])
				self.CustomizationAnimation = false
			else
				self:playAnim(tbl.to[1], tbl.from[2], 0, nil, tbl.to[1])
				self.CustomizationAnimation = true
			end
		end
