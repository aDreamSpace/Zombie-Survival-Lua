...
CustomizableWeaponry.statDisplay.registered_melee = {}
...
function fallbackFuncs:compare(wep)
	if self.reverse then
		...
		if var1 and var2 then
			if istable(var1) then var1 = var1[1] end --hacky fix for something that shouldnt exist
			if istable(var2) then var2 = var2[1] end
...
function CustomizableWeaponry.statDisplay:addStat(tbl)
	...
	if tbl.MeleeStat then
		table.insert(self.registered_melee, tbl)
	else
...
function CustomizableWeaponry.statDisplay:draw(wep, tab)
	...
	for k, v in ipairs(wep.CW20Melee and self.registered_melee or self.registered) do --HACK HACK: Really bad use of the nil or check, optimize this out eventually
		-- draw stat name
...
--MELEE STATS
local stat = {}
stat.varName = "PrimaryAttackDamage"
stat.display = "PRI. DAMAGE"
stat.desc = "Damage range of each slash attack."
stat.MeleeStat = true

function stat:textFunc(wep)
	local tbl = wep.PrimaryAttackDamage
	return tbl[1].."/"..tbl[2]
end

function stat:origTextFunc(wep)
	local tbl = wep.PrimaryAttackDamage_Orig or wep.PrimaryAttackDamage
	return tbl[1].."/"..tbl[2]
end
...
local stat = {}
stat.varName = "SecondaryAttackDamage"
stat.display = "SEC. DAMAGE"
stat.desc = "Damage range of each stab attack."
stat.MeleeStat = true

function stat:textFunc(wep)
	local tbl = wep.SecondaryAttackDamage
	return tbl[1].."/"..tbl[2]
end

function stat:origTextFunc(wep)
	local tbl = wep.SecondaryAttackDamage_Orig or wep.SecondaryAttackDamage
	return tbl[1].."/"..tbl[2]
end
...
local stat = {}
stat.varName = "PrimaryAttackRange"
stat.display = "PRIMARY RANGE"
stat.desc = "Range of the primary attack."
stat.MeleeStat = true

function stat:textFunc(wep)
	return wep.PrimaryAttackRange.."''"
end

function stat:origTextFunc(wep)
	return (wep.PrimaryAttackRange_Orig or wep.PrimaryAttackRange).."''"
end

CustomizableWeaponry.statDisplay:addStat(stat)

local stat = {}
stat.varName = "SecondaryAttackRange"
stat.display = "SECONDARY RANGE"
stat.desc = "Range of the secondary attack."
stat.MeleeStat = true

function stat:textFunc(wep)
	return wep.SecondaryAttackRange.."''"
end

function stat:origTextFunc(wep)
	return (wep.SecondaryAttackRange_Orig or wep.SecondaryAttackRange).."''"
end
