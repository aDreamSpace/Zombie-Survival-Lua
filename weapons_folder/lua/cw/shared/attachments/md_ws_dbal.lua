local att = {}
att.name = "md_ws_dbal"
att.displayName = "DBAL-A2"
att.displayNameShort = "DBAL"
att.laserRange = 4596 //4096
att.laserBeamRange = 100 // 75
...
if CLIENT then
	att.displayIcon = surface.GetTextureID("atts/md_ws_dbal")
	...
	function att:elementRender()
		local model = self.AttachmentModelsVM.md_ws_dbal.ent
		local pos = model:GetPos()
		local ang = model:GetAngles()
		
		-- rotate and prepare the position
		ang:RotateAroundAxis(ang:Right(), self.WS_DbalAngAdjust.p)
		ang:RotateAroundAxis(ang:Up(), self.WS_DbalAngAdjust.y)
		ang:RotateAroundAxis(ang:Forward(), self.WS_DbalAngAdjust.r)
		...
		local laserPos = pos + ang:Right() * self.WS_DbalPosAdjust.x + ang:Forward() * self.WS_DbalPosAdjust.y + ang:Up() * self.WS_DbalPosAdjust.z
