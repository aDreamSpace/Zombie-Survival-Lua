local att = {}
att.name = "md_ws_pistollaser"
att.displayName = "PEQ6/LAM-1400"
att.displayNameShort = "PEQ6"
att.laserRange = 4096
...
if CLIENT then
	att.displayIcon = surface.GetTextureID("atts/md_ws_pistollaser")
	...
	function att:elementRender()
		local model = self.AttachmentModelsVM.md_ws_pistollaser.ent
		local pos = model:GetPos()
		local ang = model:GetAngles()
		
		-- rotate and prepare the position
		ang:RotateAroundAxis(ang:Right(), self.WS_PistolLaserAngAdjust.p)
		ang:RotateAroundAxis(ang:Up(), self.WS_PistolLaserAngAdjust.y)
		ang:RotateAroundAxis(ang:Forward(), self.WS_PistolLaserAngAdjust.r)
		...
		local laserPos = pos + ang:Right() * self.WS_PistolLaserPosAdjust.x + ang:Forward() * self.WS_PistolLaserPosAdjust.y + ang:Up() * self.WS_PistolLaserPosAdjust.z
