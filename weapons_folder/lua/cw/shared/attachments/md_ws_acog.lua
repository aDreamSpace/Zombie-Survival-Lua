...
att.aimPos = {"WS_ACOGPos", "WS_ACOGAng"}
att.FOVModifier = 15
att.isSight = true
...
if CLIENT then
	att.displayIcon = surface.GetTextureID("atts/md_ws_acog")
	...
	att.reticle = "cw2/reticles/White_Snow/Acog"
	
	local old, x, y, ang
	local reticle = surface.GetTextureID("cw2/reticles/White_Snow/Acog")
	...
	function att:drawRenderTarget()
		...
		if not complexTelescopics then 
			Material("models/attachments/White_Snow/acog/lens"):SetTexture("$basetexture", lensMat:GetTexture("$basetexture"))
			return
		end
		...
		ang:RotateAroundAxis(ang:Right(), self.WS_ACOGAxisAlign.right)
		ang:RotateAroundAxis(ang:Up(), self.WS_ACOGAxisAlign.up)
		ang:RotateAroundAxis(ang:Forward(), self.WS_ACOGAxisAlign.forward)
		...
		render.SetViewPort(0, 0, 512, 512)
			...
			cam.Start2D()
				...
				surface.DrawTexturedRectRotated(260, 260, 750, 750, 45)
		...
		if Material("models/attachments/White_Snow/acog/lens") then
			Material("models/attachments/White_Snow/acog/lens"):SetTexture("$basetexture", self.ScopeRT)
		end
	end
end
